  ...
  initPools() {
    ...
    this.pool = {
      ...
      read: Pooling.createPool({
        ...
        validate: config.replicationPool.read.validate || config.pool.validate
      }, {
        Promise: config.replicationPool.read.Promise || config.pool.Promise,
        testOnBorrow: true,
        returnToHead: true,
        autostart: false,
        max: config.replicationPool.read.max,
        min: config.replicationPool.read.min,
        acquireTimeoutMillis: config.replicationPool.read.acquire,
        idleTimeoutMillis: config.replicationPool.read.idle,
        evictionRunIntervalMillis: config.replicationPool.read.evict
      }),
      write: Pooling.createPool({
        ...
        destroy: connection => this._disconnect(connection),
        validate: config.replicationPool.write.validate || config.pool.validate
      }, {
        Promise: config.replicationPool.write.Promise || config.pool.Promise,
        testOnBorrow: true,
        returnToHead: true,
        autostart: false,
        max: config.replicationPool.write.max,
        min: config.replicationPool.write.min,
        acquireTimeoutMillis: config.replicationPool.write.acquire,
        idleTimeoutMillis: config.replicationPool.write.idle,
        evictionRunIntervalMillis: config.replicationPool.write.evict
      })
    };
    debug(`read pool created with max/min: ${config.replicationPool.read.max}/${config.replicationPool.read.min}, with replication`);
    debug(`write pool created with max/min: ${config.replicationPool.write.max}/${config.replicationPool.write.min}, with replication`);
  }
