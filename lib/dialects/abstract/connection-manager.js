  ...
  initPools() {
    ...
    this.pool = {
      ...
      read: Pooling.createPool({
        ...
        returnToHead: true,
        autostart: false,
        max: config.replication.read[0].pool.max || config.pool.max,
        min: config.replication.read[0].pool.min || config.pool.min,
        acquireTimeoutMillis: config.replication.read[0].pool.acquire || config.pool.acquire,
        idleTimeoutMillis: config.replication.read[0].pool.idle || config.pool.idle,
        evictionRunIntervalMillis: config.replication.read[0].pool.evict || config.pool.evict
      }),
      write: Pooling.createPool({
        ...
        returnToHead: true,
        autostart: false,
        max: config.replication.write.pool.max || config.pool.max,
        min: config.replication.write.pool.min || config.pool.min,
        acquireTimeoutMillis: config.replication.write.pool.acquire || config.pool.acquire,
        idleTimeoutMillis: config.replication.write.pool.idle || config.pool.idle,
        evictionRunIntervalMillis: config.replication.write.pool.evict || config.pool.evict
      })
    };
